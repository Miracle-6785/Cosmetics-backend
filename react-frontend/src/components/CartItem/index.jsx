...
export default function CartItem() {
  ...
  const changeQty = (id, changeQuantity) => {
    if (productIdsInCartReduce.hasOwnProperty(id)) {
      ...
      const updatedQtyProducts = Object.keys(productIdsInCartReduce).flatMap(
        (productId) =>
          Array.from(
            { length: productIdsInCartReduce[productId] },
            () => productId
          )
      );
    ...
    <>
      ...
      {productIdsInCart.length == 0 ? (
        <div className="cart-item-alert">
          You have no items in cart, please add more !
        </div>
      ) : null}
      {Object.entries(productIdsInCartReduce).map(([productId, quantity]) => {
        const productItem = productInfoList.find(
        ...
        if (productItem && productIdsInCart) {
          return (
            <div
              className="cart-item"
              id={"product-" + productItem.productId}
            ...
            >
              ...
              <div className="cart-item-info">
                {productItem.name.split(" ").slice(0, 2).join(" ")}
              </div>
              <div className="cart-item-price">{productItem.productPrice}</div>
              <div className="cart-item-qty">
                <button
                  onClick={() => changeQty(productItem.productId, "decrease")}
                >
                  -
                </button>
                <div className="qty">{quantity}</div>
                <button
                  onClick={() => changeQty(productItem.productId, "increase")}
                >
                  +
                </button>
              </div>
